...
static void InitHardUart(void)
{
	...
	*/
	...
	USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;
    #if 1
        USART_InitStructure.USART_Mode = USART_Mode_Rx | USART_Mode_Tx;		/*如果是测深仪，需要收发模式。如果是深度传感器就是接收模式*/                                                                           
	#else
        USART_InitStructure.USART_Mode = USART_Mode_Rx;     /* 使用深度传感器时就仅选择接收模式 */
    #endif
...
#if UART4_FIFO_EN == 1			/* 串口4 TX = PA0   RX = PA1 */
	/* 第1步： 配置GPIO */
